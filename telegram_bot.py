...
async def switch_character(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for name, desc in characters:
        keyboard.append([InlineKeyboardButton(name, callback_data=f"character_{name}")])
    
    # Добавляем кнопку отмены
    keyboard.append([InlineKeyboardButton("Отмена", callback_data="character_cancel")])
...
async def character_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if callback_data == "character_cancel":
        await query.edit_message_text("Смена персонажа отменена.")
        return ConversationHandler.END
    
    # Извлекаем имя персонажа из callback_data
    character_name = callback_data.replace("character_", "")
...
def main() -> None:
    ...
    character_conv_handler = ConversationHandler(
        ...
        states={
            SELECTING_CHARACTER: [CallbackQueryHandler(character_selected, pattern=r'^character_')],
        },
